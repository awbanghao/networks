...
```{r}
# custom communities
presentationcommunity <- c(rep(1, 5), rep(2, 14))

presentationcommunitygroups <- lapply(unique(presentationcommunity), function(comm_num) {which(presentationcommunity == comm_num)})
names(presentationcommunitygroups) <- c("Successful Ageing", "Resilience Factors")
...
# community
, color = c("#e0f1ff", "#ffe2ef")[presentationcommunity]
, label.color = rep("#677271", length(presentationcommunity))[presentationcommunity]
, groups = presentationcommunitygroups)

plot1 %>% print()

# pdf(file = "video1.pdf", width = 8, height = 6)
# plot(plot1)
# dev.off()
# browseURL("video1.pdf")
```

```{r}
bei <- bridge$`Bridge Expected Influence (1-step)`
df <- data.frame(
  node = names(bei),
  bei = bei,
  community = bridge$communities
) %>%
  filter(community == "2") %>%  # only community 2
  mutate(
    node = reorder(node, bei),  # Order by value
    sign_group = ifelse(bei < 0, "negative", "positive")  # Color groups
  )

plot2 <- ggplot(df, aes(x = bei, y = node, fill = sign_group)) +
  geom_col(width = 0.7) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "black", size = 0.5) +
  scale_fill_manual(
    values = c("negative" = "violetred", "positive" = "steelblue"),
    name = "BEI Sign"
  ) +
  labs(
    title = "Bridge Expected Influence (BEI) with Successful Ageing",
    x = "BEI",
    y = "Resilience Factors"
  ) +
  theme_classic() +
...
# pdf(file = "video2.pdf", width = 8, height = 6)
# plot(plot2)
# dev.off()
# browseURL("video2.pdf")
```
